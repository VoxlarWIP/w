...
if ENV.PerplexWareESP then
    pcall(function()
        ENV.PerplexWareESP:Unload()
    end)
end
...
Airflow.Theme.Background  = Color3.fromRGB(10, 10, 10)
Airflow.Theme.Surface     = Color3.fromRGB(18, 18, 18)
Airflow.Theme.Surface2    = Color3.fromRGB(24, 24, 24)
Airflow.Theme.Surface3    = Color3.fromRGB(38, 38, 38)
Airflow.Theme.Stroke      = Color3.fromRGB(45, 45, 45)
Airflow.Theme.StrokeHover = Color3.fromRGB(100, 100, 100)
Airflow.Theme.Accent      = Color3.fromRGB(220, 220, 220)
Airflow.Theme.AccentDark  = Color3.fromRGB(10, 10, 10)
Airflow.Theme.Text        = Color3.fromRGB(230, 230, 230)
Airflow.Theme.Muted       = Color3.fromRGB(130, 130, 130)
Airflow.Theme.Success     = Color3.fromRGB(180, 180, 180)
Airflow.Theme.Warning     = Color3.fromRGB(200, 200, 200)
Airflow.Theme.Error       = Color3.fromRGB(160, 160, 160)

local Window = Airflow:CreateWindow({
    Name              = "PerplexWare",
    LoadingSubtitle   = "Sniper Arena",
    Icon              = "aperture",
    ...
    Loading = {
        ...
        Steps    = { "Initializing hooks", "Loading ESP engine", "Preparing UI" },
        Duration = 1.6,
    },
    ConfigurationSaving = {
        Enabled    = true,
        FolderName = "PerplexWare",
        FileName   = "default",
    },
...
local function Notify(data)
    ...
    local ok, err = pcall(function()
        Window:Notify({
            Title   = tostring(data.Title or "PerplexWare"),
            Content = tostring(data.Description or ""),
            Icon    = tostring(data.Icon or "bell"),
            Duration = tonumber(data.Duration) or 5,
        })
    end)
    if not ok then
        warn("[PerplexWare] Notification error:", err)
    end
    return ok
end
...
local function updateCham(entry, visual, espColor)
    ...
    if not entry.Cham or not entry.Cham.Parent then
        ...
        highlight.Name    = "PerplexWare_Cham"
...
local COMBAT_HOOK_STATE_KEY = "PerplexWareCombatHookState_V1"
...
local CombatHooksInstalled = installCombatHooks()
...
local ESPTab = Window:CreateTab({
    Name      = "ESP",
    Desc      = "Box, name, health,\ntracers and arrows",
    Icon      = "scan",
})

local CombatTab = Window:CreateTab({
    Name      = "Combat",
    Desc      = "Silent aim targeting.\nRisky — may be detected",
    Icon      = "crosshair",
})

local SettingsTab = Window:CreateTab({
    Name = "Settings",
    Desc = "Save and load configs",
    Icon = "settings",
})
...
ESPTab:CreateDivider()
ESPTab:CreateSection("Text & Health")
ESPTab:CreateLabel({
...
ESPTab:CreateDivider()
ESPTab:CreateSection("World Visuals")
ESPTab:CreateLabel({
...
CombatTab:CreateDivider()
CombatTab:CreateSection("Advanced")
CombatTab:CreateLabel({
...
CombatTab:CreateDivider()
CombatTab:CreateSection("Hybrid Weights")
CombatTab:CreateLabel({
...
SettingsTab:CreateLabel({
    Text = "Save your settings between sessions.\nFiles stored in workspace/PerplexWare/",
})
SettingsTab:CreateConfigManager({ Name = "Configs" })

SettingsTab:CreateDivider()
SettingsTab:CreateSection("Links")
SettingsTab:CreateLabel({
    Text = "Discord: discord.gg/4qsE29qaPJ",
})
...
function Controller:Unload()
    ...
    pcall(function() self.Library:Destroy() end)
    if ENV.PerplexWareESP == self then
        ENV.PerplexWareESP = nil
    end
end
